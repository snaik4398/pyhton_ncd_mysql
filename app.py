...
# ------------------------------------------------------------------------------------------------------------------
    ...
    cursor.execute('UPDATE pat_info SET score =% s,result =% s WHERE adhar =% s',(score,res,adhar,))
    mysql.connection.commit()


    return render_template('result.html',result=res,sc=score,kk=adhar)
...
def signup():
    ...
    cursor.execute('INSERT INTO pat_info VALUES (NULL, % s, % s, % s, % s, % s, % s, % s,% s,% s)',(name1, email1, gen1 , dob, adhar, phno, pin,sc,res, ))
    mysql.connection.commit()
